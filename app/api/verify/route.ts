...
export async function GET(request: Request) {
  ...
  const { data, error } = await documentsTable
    .select(
      "id, department, title, recipient_name, issue_date, expiry_date, storage_path, mime_type, file_size, processed_file_name, verified",
    )
    .eq("id", documentId)
    .maybeSingle();
  ...
  // If the document exists but has not been verified by an admin, report
  // it as invalid so scanners show it as non-authentic until reviewed.
  if (!(data as any).verified) {
    return NextResponse.json({ status: "invalid" });
  }

  // The storage_path points to the processed document stored in the `documents` bucket.
